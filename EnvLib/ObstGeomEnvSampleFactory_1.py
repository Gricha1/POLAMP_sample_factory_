...
class ObsEnvironment(gym.Env):
    def __init__(self, full_env_name, config):
        ...
        self.observation_space = gym.spaces.Box(obs_min_box, obs_max_box, dtype=np.float32)
        self.action_space = gym.spaces.Box(low=np.array([-self.vehicle.max_acc, -self.vehicle.max_ang_vel], dtype=np.float32), high=np.array([self.vehicle.max_acc, self.vehicle.max_ang_vel], dtype=np.float32))
        self.lst_keys = list(self.maps.keys())
        index = np.random.randint(len(self.lst_keys))
        self.map_key = self.lst_keys[index]
        self.obstacle_map = self.maps[self.map_key]
    ...
    def transformTask(self, from_state, goal_state, obstacles, dynamic_obstacles=[]):
        ...
        # print(f"len(self.dynamic_obstacles): {len(self.dynamic_obstacles)}")
        # print(f"len(self.obstacle_map): {len(self.obstacle_map)}")
    ...
    def step(self, action, next_dyn_states=[]):
        # print(f"self.stepCounter: {self.stepCounter}")
        ...
        if goalReached or collision or (self._max_episode_steps == self.stepCounter):
            ...
            # print(f"info {info}")
            # print(f"goalReached {goalReached}")
            # print(f"self.stepCounter {self.stepCounter}")
    ...
    def render(self, reward, figsize=(10, 8), save_image=True):
        ...
        ax.set_title('dx: {:.1f} dy: {:.1f} theta: {:.0f}'.format(dx, dy, theta))
            # f'$s={:.1f}, l={dy:.1f},\\alpha={theta:.0f}^\\circ, v={v:.2f} \, km/h, \\delta={delta:.0f}^\\circ, \\beta={radToDeg(self.goal.theta):.0f}^\\circ, v0={mToKm(self.goal.v):.0f} \, km/h, reward={reward:.0f}.$')
