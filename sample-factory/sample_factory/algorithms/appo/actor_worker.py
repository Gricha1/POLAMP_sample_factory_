...
class ActorState:
    ...
    def set_trajectory_data(self, data, rollout_step):
        ...
        """
        #print("debug data type:", data['obs']['obs'].keys())
        #print("debug data type:", type(data['obs']['obs']))
...
class VectorEnvRunner:
    ...
    def __init__(self, cfg, num_envs, worker_idx, split_idx, num_agents, shared_buffers, pbt_reward_shaping):
        ...
        #print("DEBUG count actor states per env:", num_agents)
    ...
    def init(self):
        ...
        for env_i in range(self.num_envs):
            ...
            #print("DEBUG env created:", env_i + 1)
    ...
    def reset(self, report_queue):
        ...
        for env_i, e in enumerate(self.envs):
            ...
            #print("DEGUB obs reset:", observations[0]['obs'].keys())
            #print("DEGUB obs reset:", type(observations[0]['obs']))
            #print("DEBUG reset obs len:", len(observations), 
            #            observations[0].keys())
            ...
            for agent_i, obs in enumerate(observations):
                actor_state = self.actor_states[env_i][agent_i]
                #print("debug new obs", obs['obs'].keys())
                #print("debug new obs", type(obs['obs']))
